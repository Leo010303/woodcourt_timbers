...
app.get('/api/uploads', requireAdmin, (req, res) => {
  try {
    const items = fs.readdirSync(UPLOAD_DIR).map(fn => ({ 
      filename: fn, 
      url: `/uploads/${fn}` 
    }));
    res.json({ items });
...
// Products with prices - Load from file or use default
let products = [];
...
// Upload media to specific product
app.post('/api/products/:id/media', requireAdmin, upload.array('files', 20), (req, res) => {
  const productId = parseInt(req.params.id);
  const productIndex = products.findIndex(p => p.id === productId);
  ...
  // Add files to product's media array
  products[productIndex].media.push(...files);
  saveProducts();
